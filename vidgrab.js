	...
	return {
		...
		download: function(vine_id, options) {
			...
			request(r_options, function(err, response, body) {
				if(err) throw err; 
				if(response.statusCode === 404) throw '404: Video Not Found'; // if 404, stop processing
				...
				if(links) {
					...
					if(cb) cb(r);
					// or write to default
					else r.pipe(require('fs').createWriteStream(dir + vine_id + '.mp4'));
				}
				else throw 'Failed to find a match';
			});	
		}
	}
})();